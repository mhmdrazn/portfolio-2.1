...
const blog = [
  {
    ...
    description:
      'Memory is one of the important elements in a computer that has a central role in processing data and programs. We can liken this memory to computer memory. Its function is to store data that will be used by the computer when operating.',
  ...
  },
  {
    ...
    description:
      'If we develop a small program for our CPU that allows division of two numbers by repeatedly performing subtraction, the process would look like this: for example, to divide 16 by 4, we repeatedly subtract 4 from 16 until we reach zero or a negative number.',
...
export default function Home() {
  return (
    ...
    >
      <Template>
        <div className="flex flex-col gap-4 items-start w-full justify-start">
          ...
          <div className="flex flex-col gap-2">
            <h1 className="font-medium text-lg">Muhammad Razan</h1>
            <p className="anim text-background/70 text-xs md:text-sm">
              A student by day, a creative thinker, and a moody writer by night. Sometimes do
              design, sometimes code, and sometimes write.
            </p>
          </div>
                ...
                <Link className="flex flex-col gap-3" key={key} href={index.href}>
                  <div className="w-5 h-5 text-background/50 hover:text-background anim">
                    <TooltipProvider delayDuration={0}>
                      <Tooltip>
                        <TooltipTrigger asChild>{index.icon}</TooltipTrigger>
      ...
      <Template>
        <h1 className="text-md font-semibold">About Me</h1>

        <Paragraph
          text={
            "I'm Muhammad Razan, but you can call me Razan. Currently an undergraduate Information Systems student @ITS. I'm a curious designer, coder, and maybe even a storyteller (though I’m not so sure about that, lol). I have a strong interest in Product Management, Software Development, and Design. Based in Surabaya, Indonesia."
          }
        />

        <div className="flex flex-col items-start justify-center w-full">
          <h1 className="text-xs text-background/70">Still curious? Ask me anything.</h1>
          <Chat />
        </div>
      </Template>
      {/* 
      ...
      <Template>
        <h1 className="text-md font-semibold">Where am I currently?</h1>
      ...
      <Template>
        <h1 className="font-semibold">Toolkit and Weapons</h1>

        <Paragraph
          text={
            "When I'm not busy saving the digital world one pixel at a time, I arm myself with these tools. Think of me as a digital superhero, but instead of a cape, I wear a hoodie, and instead of a shield, I wield a keyboard. Here’s what’s in my utility belt."
          }
        />
          ...
          <InfiniteSlider durationOnHover={125} gap={24}>
            <div className="flex flex-row gap-8">
              {techStacks.map((index, key) => (
                <div
                  key={key}
                  className="flex flex-row gap-2 items-center text-xs text-background/70 anim h-6"
                >
                  <div className="w-4 h-4">{index.icon}</div>
                  {index.name}
                </div>
              ))}
            </div>
      ...
      <Template>
        <div className="text-md font-semibold flex flex-row justify-between w-full items-center">
          Read my mind
          <Link
            className="text-background/70 hover:text-background text-xs anim w-fit"
      ...
      <Template>
        <div className="text-md font-semibold flex flex-row justify-between w-full items-center">
          Side Projects
          <Link
            className="text-xs anim text-background/70 hover:text-background w-fit hover:text-background/"
            href={'/projects'}
          >
            See more
          </Link>
        ...
        <Paragraph
          text={
            'These are my passion projects, where I turn caffeine into code and dreams into apps. Think of them as my digital children, sometimes dysfunctional, but always full of potential.'
          }
        />
      ...
      <Template>
        ...
        <h1 className="text-background/50 text-xs md:text-sm w-fit text-justify">
          This page was crafted by Razan, drawing inspiration from{' '}
          <span className="text-background/70 anim hover:text-background">
            <Link href={'https://msaf.tech'}>@msaf.tech</Link>
          </span>
          . Some components are sourced from{' '}
          <span className="text-background/70 anim hover:text-background">
            <Link href={'https://ui.msaf.tech'}>ui.msaf.tech.</Link>
          </span>
        </h1>
      </Template>
    </main>
