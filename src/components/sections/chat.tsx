    ...
    <div className="md:max-w-lg max-w-md w-full h-fit flex flex-col items-start gap-2 mt-2">
      ...
      <div className="flex flex-row gap-2 w-full">
        <Input
          ...
          disabled={loading}
        />
        <Button disabled={loading} onClick={handleSend} type="submit" className="w-fit anim">
      ...
      {response && (
        <div className="anim pt-2">
          ...
          <div className="mt-1 text-sm text-background/80 w-full text-justify">{response}</div>
        </div>
      )}
    </div>
  );
