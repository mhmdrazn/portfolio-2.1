...
const BentoGrid = ({ children, className, ...props }: BentoGridProps) => {
  return (
    <div
      className={cn('grid w-full auto-rows-[13rem] md:auto-rows-[17rem] grid-cols-2', className)}
      {...props}
    >
      {children}
    </div>
...
const BentoCard = ({ name, className, background, desc, href, logo, ...props }: BentoCardProps) => (
  <div
    key={name}
    className={cn(
  ...
  >
    ...
    <div className="pointer-events-none z-10 flex transform-gpu flex-col gap-1 p-4 md:p-6 transition-all duration-300 group-hover:-translate-y-10">
      <Image
        ...
        width={24}
        height={24}
        className="origin-left top-0 transform-gpu text-neutral-700 transition-all duration-300 ease-in-out group-hover:scale-75"
      />
